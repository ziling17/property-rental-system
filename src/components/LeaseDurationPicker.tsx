      ...
      {isValid ? (
        <div className="mt-6 p-4 bg-surface-container rounded-lg flex items-start gap-3 border border-primary/10 animate-in fade-in slide-in-from-bottom-2 duration-150">
          ...
          <p className="text-xs md:text-sm text-on-surface-variant leading-relaxed">
            Minimum lease duration for this property is 6 months. Currently selected: <strong className="text-primary">{durationMonths} months</strong>. Dates are subject to final landlord approval during the verification process.
          </p>
        </div>
      ) : (
        <div className="mt-6 p-4 bg-error-container text-on-error-container rounded-lg flex items-start gap-3 border border-error/20 animate-wiggle duration-300">
          ...
          <div className="text-xs md:text-sm">
            <p className="font-bold mb-0.5">Invalid Lease Duration</p>
            <p className="leading-relaxed text-on-error-container/90">
              The minimum lease duration required is <strong>6 months</strong>. Your selection is only <strong>{durationMonths} month{durationMonths !== 1 ? 's' : ''}</strong>. Please adjust the end date to satisfy standard compliance.
            </p>
          </div>
        </div>
      )}
